...
## Tests and checks
- Required for every PR: `zig fmt --check src/ examples/ build.zig`, `zig build quality`, `python3 scripts/check_accessibility_metadata.py`, `python3 scripts/check_application_input_binding.py`, `python3 scripts/check_docs_commands.py`, `python3 scripts/check_docs_links.py`, `python3 scripts/check_docs_zig_snippets.py`, `python3 scripts/check_draw_layout_boundary.py`, `python3 scripts/check_example_coverage.py`, `python3 scripts/check_interactive_alt_screen.py`, `python3 scripts/check_io_event_ownership_docs.py`, `python3 scripts/check_mouse_coordinate_contract.py`, `python3 scripts/check_mouse_hit_coverage.py`, `python3 scripts/check_owned_allocation_patterns.py`, `python3 scripts/check_terminal_state_cleanup.py`, `python3 scripts/check_unreachable_catches.py`, `python3 scripts/check_widget_owner_casts.py`, `python3 scripts/check_widget_lifecycle_mutation.py`, and `python3 scripts/check_widget_parent_attachment.py`.
...
- If Zig is installed outside `PATH`, set `ZIG` to the executable path; release helper scripts also accept `--zig` for one-off runs.
- TUI-facing changes need `python3 scripts/interactive_example_smoke.py`, `python3 scripts/resize_smoke.py --no-build`, `python3 scripts/mouse_alignment_smoke.py --no-build`, and `python3 scripts/visual_repeat_check.py --count 4` with contact-sheet inspection.
...
- Unicode grapheme-property updates must use `python3 scripts/generate_unicode_grapheme_data.py`; commit the regenerated table, official fixture, and license together, then run `zig build test` and `zig build release-check`.
