...
- Full release verification is `zig build release-check`; it runs quality, formatting, docs generation, public build steps, cross-target smoke, interactive PTY smoke, resize PTY smoke requiring every public interactive example to survive rapid tiny-size stress and redraw a live `resize: WxH` marker, mouse hit-test coverage checks, memory cleanup checks, and visual repeat captures.
