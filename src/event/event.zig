...
pub const EventDispatcher = struct {
    ...
    pub fn addEventListener(self: *EventDispatcher, event_type: EventType, listener: EventListenerFn, user_data: ?*anyopaque) !u32 {
        ...
        try self.listeners.append(EventListener{
...
pub const EventQueue = struct {
    ...
    pub fn createCustomEventWithFilter(
        self: *EventQueue, 
        id: u32, 
        data: ?*anyopaque, 
        destructor: ?*const fn (data: *anyopaque) void, 
        type_name: ?[]const u8,
        filter_fn: ?*const fn (data: ?*anyopaque) bool,
...
pub const FocusManager = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, event_queue: *EventQueue) FocusManager {
        return FocusManager{
            .focused_widget = null,
            .focus_history = std.ArrayList(*widget.Widget).init(allocator),
    ...
    pub fn requestFocus(self: *FocusManager, target_widget: *widget.Widget) !bool {
        ...
        if (!self.allow_focus_stealing and self.focused_widget != null) {
            ...
            try self.event_queue.createCustomEventWithFilter(
                FOCUS_REQUEST_EVENT_ID,
                @ptrCast(&focus_data),
                null,
                "focus_request",
                null,
        ...
        try self.focus_history.append(target_widget);
    ...
    pub fn processInputEvent(self: *Application, input_event: input.Event) !void {
        ...
        switch (input_event) {
            .key => |key_event| {
                ...
                try self.event_queue.createKeyPressEvent(
                    key_event.key,
                    key_event.modifiers,
                    0, // We don't have raw key codes in our input system
                    @ptrCast(self.root.?)
                );
            },
