...
- **Layout pass → Draw pass**
  ...
  - Fallible geometry preparation and retained-cache growth happen at layout or mutation boundaries. Production draw callbacks consume that state without child layout or heap growth; `scripts/check_draw_layout_boundary.py` enforces the boundary recursively for local helpers.
