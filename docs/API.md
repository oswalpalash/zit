...
## Core Modules
- `terminal` – `Terminal.init(allocator)`, `enableRawMode/disableRawMode`, terminal-owned `enableMouseEvents/disableMouseEvents`, `moveCursor`, `clear`, `enterAlternateScreen`, `beginSynchronizedOutput/endSynchronizedOutput`, and `reportCleanupError(action, err)` for deferred cleanup paths that cannot return errors.
- `input` – `InputHandler.init(allocator, &terminal)`, terminal-delegating `enableMouse/disableMouse/isMouseEnabled`, `pollEvent(timeout_ms)`, resize detection via SIGWINCH plus periodic geometry polling, decoded mouse events in zero-based render coordinates, Kitty flag-1 CSI-u keys with Shift/Alt/Ctrl modifiers, bracketed paste delimiters as `KeyCode.BRACKETED_PASTE_START` / `KeyCode.BRACKETED_PASTE_END`, plus key codes (`KeyCode.*`) and modifiers.
