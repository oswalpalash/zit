...
- `event.Application` – event loop coordinator with owned background-run handles (`spawnRunLoop`/`RunLoopHandle.join`), timers with overflow-safe deadlines, animations, background tasks, shortcuts, accessibility, `bindInput(&input)`, and transactional `bindResize(&renderer, &reflow)` handling that publishes renderer and widget geometry together.
...
- `render` – `Renderer.init(allocator, width, height)`, atomic `resize`, scoped `prepareResize`/`ResizeTransaction.commit` for coordinating other size-dependent state, `drawStr`, `drawBox`, `fillRect`, `drawGradient`, `measureText`, allocation-free `clipTextToWidth`, and `render()` (transactional steady-state allocation-free front/back diff). Colors via `Color.named/rgb/ansi256`, styles via `Style` and `FocusRingStyle`.
