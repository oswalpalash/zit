...
- **Windows VT negotiation**: Output processing and input mode are tracked independently. Mouse, focus, bracketed paste, and Kitty keyboard setup require both; `supportsVtInputProtocols()` reports the negotiated path. Output-only synchronized rendering still works when input VT is unavailable. If initialization enables VT output or raw-mode setup changes console modes, restoration is owned through constructor rollback or `deinit`, even when later setup fails.
