...
- **Fragmented input**: `InputHandler` waits up to 25 ms by default for each byte continuing an ESC, CSI, mouse, or UTF-8 sequence, allowing input split across SSH, tmux, PTY, and Windows console reads to decode as one event. POSIX uses `poll`; Windows uses its waitable input handle. Already-buffered bytes remain immediate. `setSequenceTimeout` adjusts the bound; `0` restores immediate continuation reads. A lone Escape can therefore take up to the configured timeout to distinguish from an Alt chord or escape sequence.
