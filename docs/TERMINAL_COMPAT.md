...
- **POSIX resize signals**: Zit installs a reference-counted SIGWINCH handler while `Terminal` instances are live and restores the prior handler after the last `Terminal.deinit`. A failed geometry refresh retains the pending notification so callers can retry instead of losing the resize event.
