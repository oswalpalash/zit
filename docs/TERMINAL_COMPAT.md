...
- **ConPTY resizes**: A signaled Windows console handle does not prove that readable VT bytes are present because `WINDOW_BUFFER_SIZE_EVENT` also occupies the input queue. Before blocking on bytes, Zit drains consecutive leading resize records, refreshes geometry immediately, and only reads bytes when another input record remains.
