...
- **ConPTY synthetic records**: A signaled Windows console handle does not prove that readable VT bytes are present. Zit peeks the queue before blocking: resize records refresh geometry, focus records become terminal focus events, menu/private records are discarded safely, and keyboard/mouse records remain for VT-byte decoding in queue order.
- **Windows VT negotiation**: Raw mode explicitly requests `ENABLE_WINDOW_INPUT` so ConPTY resize/focus records do not depend on an inherited console mode. Output processing and input mode are tracked independently. Mouse, focus, bracketed paste, and Kitty keyboard setup require both VT modes; `supportsVtInputProtocols()` reports the negotiated path. Output-only synchronized rendering still works when input VT is unavailable. If initialization enables VT output or raw-mode setup changes console modes, restoration is owned through constructor rollback or `deinit`, even when later setup fails.
