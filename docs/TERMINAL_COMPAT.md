...
- **Fragmented POSIX input**: `InputHandler` waits up to 25 ms by default for each byte continuing an ESC, CSI, mouse, or UTF-8 sequence, allowing input split across SSH, tmux, and PTY reads to decode as one event. Already-buffered bytes remain immediate. `setSequenceTimeout` adjusts the bound; `0` restores immediate non-blocking continuation reads. A lone Escape can therefore take up to the configured timeout to distinguish from an Alt chord or escape sequence.
