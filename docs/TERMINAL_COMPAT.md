...
- **POSIX raw mode and geometry**: macOS and Linux use direct termios mode changes and their native `TIOCGWINSZ` ABIs; avoid mixing external `stty` or terminal-mode changes while Zit is active. macOS uses the Darwin ABI, while the release-supported Linux matrix uses the Linux ABI.
- **Geometry fallback**: runtime refreshes use only live terminal queries (`TIOCGWINSZ` or Windows console buffers). A failed query reports `TerminalGeometryUnavailable` and preserves the previous cached width/height. `COLUMNS` and `LINES` are transactional initialization fallbacks for hosts where the initial probe cannot answer, never stale replacements for a later failed probe.
- **POSIX resize signals**: Zit installs a reference-counted SIGWINCH handler while `Terminal` instances are live and restores the prior handler after the last `Terminal.deinit`. Generation accounting acknowledges only an uninterrupted successful refresh: failed refreshes remain retryable, and a signal arriving during a query remains pending instead of being folded into stale geometry.
- **Fragmented input**: `InputHandler` waits up to 25 ms by default for each byte continuing an ESC, CSI, mouse, or UTF-8 sequence, allowing input split across SSH, tmux, PTY, and Windows console reads to decode as one event. POSIX uses `poll`; Windows uses its waitable input handle. Already-buffered bytes remain immediate. `setSequenceTimeout` adjusts the bound; `0` restores immediate continuation reads. A lone Escape can therefore take up to the configured timeout to distinguish from an Alt chord or escape sequence. Timeouts remain ordinary no-event/incomplete-sequence results, while closed or invalid input handles and poll/read failures propagate as errors.
- **PTY file status**: POSIX raw mode uses termios `VMIN=0`/`VTIME=0` and `poll` for bounded input waits. It does not set `O_NONBLOCK`, because PTY stdin and stdout commonly refer to the same open file description and changing stdin flags can make renderer writes fail with `WouldBlock` under output pressure.
...
- **ConPTY synthetic records**: A signaled Windows console handle does not prove that readable VT bytes are present. Zit peeks the queue before blocking: resize records refresh geometry, focus records become terminal focus events, menu/private and horizontal-wheel records are discarded safely, native mouse records map directly to zero-based widget events, and keyboard records remain for VT-byte decoding in queue order.
- **Windows VT negotiation**: Raw mode explicitly requests `ENABLE_WINDOW_INPUT` and `ENABLE_MOUSE_INPUT` so console records do not depend on inherited modes. Output processing and input mode are tracked independently. Focus, bracketed paste, Kitty keyboard, and VT-encoded mouse setup require both VT modes; when either is unavailable, mouse tracking can use native Win32 records after raw mode. `supportsVtInputProtocols()` reports whether those VT-only paths are available. Output-only synchronized rendering still works when input VT is unavailable. If initialization enables VT output or raw-mode setup changes console modes, restoration is owned through constructor rollback or `deinit`, even when later setup fails.
