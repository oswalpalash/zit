...
- `Toolbar` formats item delimiters directly into the renderer, and `Breadcrumbs` computes its visible range without temporary collections. `Table` reserves sorting/grouping view capacity at row, grouping, and provider boundaries; `groupBy`, `useRowProvider`, and `refreshRowProvider` report allocation failure before publishing the new state, while dirty view rebuilds remain allocation-free during draw.
