...
- `python3 scripts/resize_smoke.py --no-build` to verify `input_test` receives a live PTY resize, every public interactive example redraws a visible `resize: WxH` marker at the new geometry, and each example quits cleanly.
