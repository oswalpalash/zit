...
- Windows raw mode must explicitly request `ENABLE_WINDOW_INPUT`; ConPTY resize/focus record delivery cannot rely on callers having inherited that mode. Original input/output console modes remain the sole restoration authority.
- Native Windows fallback mouse delivery is available only while raw mode owns the input handle; enabling it outside that lifecycle returns `RawModeRequired`, and original input/output console modes remain the sole restoration authority.
- Windows readiness must classify every console input record before blocking on bytes. Resize records refresh geometry, focus records map to terminal-focus events, unknown/menu records are consumed without inventing bytes, and only keyboard/mouse records enter blocking VT-byte decoding. Win32 input-record layout/classification remains regression-tested.
...
- Mouse decoders must preserve Shift/Alt/Ctrl modifiers across SGR, legacy, and native Windows records, and application event conversion must propagate those modifiers rather than resetting them. Win32 release records preserve the tracked button so handlers see the same button identity as SGR releases.
- TextArea multi-click recognition must require the same grapheme boundary within its timing window, reset when focus/visibility/enabled state changes, expand double selections around whitespace separators without splitting clusters, and constrain triple selections to the logical line excluding its newline.
- TextArea multi-click drag extension must retain the original token/line range as its base range, union it with the pointer target in the same selection unit, and preserve grapheme boundaries while auto-scrolling.
- TextArea drag selection must map out-of-viewport pointers through the same grapheme/terminal-cell geometry, preserve grapheme boundaries, and refresh scrolling from the resulting caret so edge drags cannot silently stall selection.
...
- SIGWINCH acknowledgement must use generation accounting: acknowledge only an uninterrupted successful refresh, preserve failed refreshes for retry, and keep signals that arrive during a query pending rather than coalescing them into stale geometry. Terminal cursor sequences widen one-based u16 coordinates before arithmetic so valid maximum geometry cannot overflow.
...
- `python3 scripts/check_draw_layout_boundary.py` recursively rejects child layout calls, allocator-backed growth, byte-prefix clipping, and indexed byte iteration over arbitrary text from production widget draw callbacks so redraws cannot republish geometry, fail after successful setup, split UTF-8 graphemes, or advance by bytes instead of terminal cells.
...
- `Toolbar`, `Breadcrumbs`, `TabBar`, and `DropdownMenu` use grapheme-safe clipping and terminal-cell widths for preferred size, draw cursors, and mouse hit regions without temporary allocation. Breadcrumb overflow reserves its marker and separator before admitting suffix segments. `Table` clips headers and cells by terminal width and reserves sorting/grouping view capacity at row, grouping, and provider boundaries; `groupBy`, `useRowProvider`, and `refreshRowProvider` report allocation failure before publishing the new state, while dirty view rebuilds remain allocation-free during draw.
- `Snapshot.expectWellFormed` accepts serialized grapheme-cell shape while retaining a codepoint-count fallback for cell boundaries that plain snapshot text cannot encode, so combining clusters, CJK, and joined emoji can participate in deterministic snapshot checks.
