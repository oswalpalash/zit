...
- `Toolbar` and `Breadcrumbs` use grapheme-safe clipping and terminal-cell widths for preferred size, draw cursors, and mouse hit regions without temporary allocation. Breadcrumb overflow reserves its marker and separator before admitting suffix segments. `Table` reserves sorting/grouping view capacity at row, grouping, and provider boundaries; `groupBy`, `useRowProvider`, and `refreshRowProvider` report allocation failure before publishing the new state, while dirty view rebuilds remain allocation-free during draw.
