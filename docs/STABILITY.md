...
- `TextArea.draw` merges its sorted primary and extra cursor positions with a stack-only iterator; focused multi-cursor rendering must not allocate. Multi-cursor backspace/delete must remove complete UTF-8 codepoints at every caret rather than individual bytes.
