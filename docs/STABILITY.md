...
- Failed `Widget.layout` calls must restore the widget's public rect and dirty-region state and must not publish failed accessibility bounds. `Container`, `FlexContainer`, `GridContainer`, `TabView`, and `ScreenManager` preflight reusable snapshot storage and restore direct-child public layout state plus previously published accessibility bounds when a later child fails; `TabView` covers all loaded content, including inactive tabs, and `ScreenManager` must also restore its cached last layout rect. Other composite vtables remain responsible for equivalent child and private-cache consistency.
