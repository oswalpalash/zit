...
- Windows readiness must distinguish readable VT input from ConPTY `WINDOW_BUFFER_SIZE_EVENT` records. Leading resize records are drained and geometry refreshed before any blocking byte read, preventing a resize-only wake-up from stalling the input loop. Win32 input-record layout/classification remains regression-tested.
