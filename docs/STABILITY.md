...
- Failed `Widget.layout` calls must restore the widget's public rect and dirty-region state and must not publish failed accessibility bounds. `Container`, `FlexContainer`, and `GridContainer` preflight reusable snapshot storage and restore direct-child public layout state plus previously published accessibility bounds when a later child fails; other composite vtables remain responsible for equivalent child and private-cache consistency.
