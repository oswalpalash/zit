...
### Fixed
- Remaining production widget draw paths that byte-clipped arbitrary labels now use terminal-cell geometry across popup, block, menu bar, status, table, chart, gauge, toast, log, indicator, and advanced controls, preserving CJK, combining, and joined emoji graphemes. The recursive draw-boundary quality gate now rejects byte-prefix clipping in addition to draw-time layout and allocation.
